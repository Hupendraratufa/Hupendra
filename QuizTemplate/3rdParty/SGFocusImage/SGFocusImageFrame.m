...
- (void)moveToTargetPosition:(CGFloat)targetX
{
    ...
    NSLog(@"pagging %d", (int)(_scrollView.contentOffset.x / _scrollView.frame.size.width));
    
    
    if (_pageControl.currentPage == 0)
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"showTimer" object:nil userInfo:nil];
    }
    else
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"removeTimer" object:nil userInfo:nil];
    }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    NSLog(@"scroll view end scrolling %d", (int)(scrollView.contentOffset.x / scrollView.frame.size.width));
    
    int intPageValue=(int)(scrollView.contentOffset.x / scrollView.frame.size.width);
    
    NSLog(@"xxxx %f",scrollView.contentOffset.x);
